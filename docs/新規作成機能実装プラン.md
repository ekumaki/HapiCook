...
## Phase 5: セキュリティ強化 🟠 次に着手

現在のFirestoreとStorageのセキュリティルールは「テストモード」（誰でも読み書き可能）のまま。
ストア公開前に必ず対応が必要。

### Firestoreセキュリティルール
- [ ] ユーザーは自分のレシピのみ読み書き可能にする（`userId == request.auth.uid`）
- [ ] `users` コレクションのアクセス制御（自分のドキュメントのみ）
- [ ] 未認証ユーザーのアクセス拒否

### Storageセキュリティルール
- [ ] ユーザーは自分のフォルダ（`users/{uid}/`）のみ読み書き可能にする
- [ ] アップロードファイルサイズの上限設定（例: 5MB）
- [ ] 画像ファイル形式の制限（image/jpeg, image/png のみ）

### API キーの保護
- [ ] Firebase API キーの使用制限設定（Google Cloud Console）
- [ ] Gemini API キーに対するリファラ制限の検討
